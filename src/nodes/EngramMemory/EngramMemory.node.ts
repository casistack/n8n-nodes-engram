...
export class EngramMemory implements INodeType {
	description: INodeTypeDescription = {
		...
		properties: [
			...
			// Backend selection
			{
				...
				options: [
					...
					},
					{
						...
						value: 'neo4j',
						description:
							'Remote Neo4j graph database. Requires Neo4j server and credentials.',
					},
				],
				default: 'embedded',
			...
			// Context Window
			{
				...
				default: 10,
				description:
					'Number of recent conversation turns to include as context for the AI',
			},
			// Extraction settings
			{
			...
			},
			{
				...
				description:
					'Minimum relevance score (0-1) for facts to be included in context',
				displayOptions: {
					show: {
						enableExtraction: ['enabled'],
					},
			...
			},
			{
				...
				description:
					'Comma-separated list of entity types to extract from conversations',
				displayOptions: {
					show: {
						enableExtraction: ['enabled'],
					},
			...
			},
			{
				...
				description: 'Number of hops to walk from matched entities. 1 = direct neighbors only, 2 = neighbors of neighbors, etc.',
			},
			// Retention settings
			{
			...
			},
			{
				...
				description:
					'Number of days or maximum episodes to keep (depends on retention type)',
				displayOptions: {
					hide: {
	...
	async supplyData(
		...
		const enableExtraction = this.getNodeParameter(
			'enableExtraction',
			itemIndex,
			'disabled',
		) as string === 'enabled';
		const maxFactsPerQuery = this.getNodeParameter(
			'maxFactsPerQuery',
			itemIndex,
			10,
		) as number;
		const minRelevanceScore = this.getNodeParameter(
			'minRelevanceScore',
			itemIndex,
			0.5,
		) as number;
		const retentionType = this.getNodeParameter(
			'retentionType',
			itemIndex,
			'forever',
		) as string;
		const retentionValue = this.getNodeParameter(
			'retentionValue',
			itemIndex,
			30,
		) as number;
		...
		try {
			...
			if (enableExtraction) {
				...
				entityTypes = entityTypesRaw.split(',').map((t) => t.trim()).filter(Boolean);
			}

			// Build embedding config if enabled
			let embeddingConfig;
			if (enableExtraction && llmConfig) {
				const enableEmbeddings = this.getNodeParameter(
					'enableEmbeddings',
					itemIndex,
					'disabled',
				) as string === 'enabled';
			...
			let traversalHops = 1;
			if (enableExtraction) {
				enableTraversal = this.getNodeParameter(
					'enableTraversal',
					itemIndex,
					'disabled',
				) as string === 'enabled';
				if (enableTraversal) {
					traversalHops = this.getNodeParameter(
						'traversalHops',
						itemIndex,
						1,
					) as number;
