		...
		properties: [
			...
			// Entity operations
			{
				...
				options: [
					...
					{ name: 'Delete', value: 'delete', description: 'Delete an entity by UUID', action: 'Delete an entity' },
					{ name: 'Get', value: 'get', description: 'Get an entity by UUID', action: 'Get an entity' },
					{ name: 'Get by Name', value: 'getByName', description: 'Get an entity by name', action: 'Get an entity by name' },
					{ name: 'List', value: 'list', description: 'List entities in a group', action: 'List entities' },
					{ name: 'Search', value: 'search', description: 'Search entities by text', action: 'Search entities' },
					{ name: 'Update', value: 'update', description: 'Update an existing entity', action: 'Update an entity' },
				],
				default: 'search',
			},
			// Relationship operations
			{
				...
				options: [
					{ name: 'Create', value: 'create', description: 'Create a new relationship between entities', action: 'Create a relationship' },
					{ name: 'Delete', value: 'delete', description: 'Delete a relationship by UUID', action: 'Delete a relationship' },
					{ name: 'Get', value: 'get', description: 'Get a relationship by UUID', action: 'Get a relationship' },
					{ name: 'Get Between', value: 'getBetween', description: 'Get relationships between two entities', action: 'Get relationships between entities' },
					{ name: 'Get for Entity', value: 'getForEntity', description: 'Get all relationships for an entity', action: 'Get relationships for entity' },
					{ name: 'Search', value: 'search', description: 'Search relationships by text', action: 'Search relationships' },
					{ name: 'Update', value: 'update', description: 'Update an existing relationship', action: 'Update a relationship' },
				],
				default: 'search',
			},
			// Episode operations
			{
				...
				options: [
					{ name: 'Get', value: 'get', description: 'Get an episode by UUID', action: 'Get an episode' },
					{ name: 'Get Count', value: 'getCount', description: 'Get episode count for a group', action: 'Get episode count' },
					{ name: 'Get Recent', value: 'getRecent', description: 'Get recent episodes', action: 'Get recent episodes' },
				],
				default: 'getRecent',
			},
			// Traversal operations
			{
				...
				options: [
					{ name: 'BFS from Entity', value: 'bfsFromEntity', description: 'Breadth-first traversal from one or more seed entities', action: 'BFS from entity' },
					{ name: 'BFS from Episodes', value: 'bfsFromEpisodes', description: 'Traverse from entities referenced in recent episodes', action: 'BFS from episodes' },
				],
...
async function executeRelationshipOperation(
	...
	if (operation === 'create') {
		...
		if (!sourceUuid || !targetUuid) {
			throw new NodeOperationError(
				ctx.getNode(),
				'Both Source UUID and Target UUID are required',
				{ itemIndex: i },
			);
		}
...
async function executeTraversalOperation(
	...
	if (operation === 'bfsFromEntity') {
		...
		const seedUuids = seedUuidsRaw.split(',').map((s) => s.trim()).filter(Boolean);

		if (seedUuids.length === 0) {
			throw new NodeOperationError(ctx.getNode(), 'At least one Seed Entity UUID is required', { itemIndex: i });
		}
